    ...
    <T as TryInto<usize>>::Error: std::fmt::Debug,
{
    ...
    pub fn from_cycles(cycles: Vec<Vec<T>>, size: usize) -> Self {
    ...
    pub fn get_subgroup_size(&mut self) -> usize {
    ...
    pub fn evaluate(&self, in_value: &T) -> Option<T> {
        self.get(in_value.clone().try_into().unwrap()).map(T::clone)
    }

    /// Apply the permutation to the given iterator.
    pub fn apply<I>(self, iterator: I) -> impl Iterator<Item = T>
    where
        I: Iterator<Item = T>,
    {
        iterator.map(move |value| self.evaluate(&value.clone()).unwrap_or(value))
    }
    ...
    fn from_cycle(cycle: &[T], raw: &mut [T]) {
...
mod test {
    ...
    fn apply_test() {
        ...
        let permuted_data: Vec<usize> = perm.apply(data.into_iter()).collect();
